...
async function cargarPerfilRemoto(userId) {
    try {
        const res  = await fetch(`../../../api/get-profile.php?id=${encodeURIComponent(userId)}`);
...
async function abrirInvite(nombre, tipo) {
    ...
    try {
        const res  = await fetch('../../../api/get-my-tournaments.php?estado=en_creacion');
        const data = await res.json();
...
async function enviarInvitacion(torneoId, btn) {
    ...
    try {
        const res  = await fetch('../../../api/invite-tournament.php', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({
                torneo_id:   torneoId,
